...
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/thinking-models-planning.md

For calibration on scoring and issue identification, reference these examples:
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/few-shot-examples/plan-checker.md
...
Anchored to `.planning/phases/28-clean-reimport/AUDIT-2026-05-08/learnings/SESSION-MISTAKES-CATALOGUE.md`. Plan-checker's job is the LAST gate before plan-MD ships — these traps must be caught here.

### AP-1 Handoff-freshness — re-derive every count cited by the plan

**Phase 28 anchor:** B4 — PT4 plan-MD's TL;DR said "Total expected stamps: 1,318 (100% of NULL universe)". Reality: 1,226 stamps + 92 ambiguous residue. Plan-MD wasn't re-aligned after PT1 finished publishing the bridge CSV.

**Rule:** when a plan-MD cites a count drawn from another doc / CSV / handoff, treat it as STALE BY DEFAULT and re-derive against the live source before approving. If the plan declares a `inputs:` field listing CSVs + expected row counts, verify each CSV's actual row count matches.

**Apply:** in plan-checker's "issues" section, raise a `STALE_COUNT` issue when any plan-MD count cannot be re-derived OR when the plan's `inputs:` field is missing for non-trivial CSV inputs.

### AP-2 Apples-to-oranges — reject bare percentages in plan-MDs

**Phase 28 anchor:** A2 — bare percentages in HANDOFF caused a phantom regression claim. Plan-MDs are the next-most-cited surface; same trap applies.

**Rule:** every coverage % in a plan-MD must carry `(numerator/denominator)` inline + the comparison axis (move-level vs line-level, sub-domain filter, time window). Refuse plan-MDs where two %s are quoted in the same sentence with different denominators.

**Apply:** add `BARE_PERCENTAGE` to plan-checker's issue taxonomy. Companion skill: `metric-filter-inline`.

### AP-3 Audit-doc decay — sequence-gate plan-MDs on dependent CSV publication

**Phase 28 anchor:** B4 — PT4 plan-MD's TL;DR was authored before PT1's bridge CSV row count was finalized. Plan-MD shipped with wrong total.

**Rule:** if a plan-MD claims to operate on the output of another phase / sibling task / wave-N teammate, the plan-MD must declare `inputs:` with explicit CSV / dataset paths + expected row counts AND the upstream task must be marked `completed` AND the CSV must exist with the expected row count.

**Apply:** raise `MISSING_UPSTREAM_GATE` when a plan-MD operates on output that is not yet published.

### AP-4 — verify FK / column claims under 90% populated coverage

**Phase 28 anchor:** A3 — proposed FK was 17.3% populated, would have produced near-zero stamps. Plan would have shipped with a bad join key.

**Rule:** when a plan-MD names a column / FK as a join key, raise an issue if the plan does not cite per-sub-domain populated coverage. Mandate the SQL coverage check before approval.
