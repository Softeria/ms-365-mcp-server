...
<required_reading>
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/common-bug-patterns.md
</required_reading>
...
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/debugger-philosophy.md
...
Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = ${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done
                  → writes to ${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/hooks/
...
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/thinking-models-debug.md
...
- Read @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/common-bug-patterns.md
...
Anchored to `.planning/phases/28-clean-reimport/AUDIT-2026-05-08/learnings/SESSION-MISTAKES-CATALOGUE.md`.

### AP-1 Estimate-grep-first — never accept an LOC / file-count / occurrence estimate without grepping

**Phase 28 anchor:** A4 — UI-gap audit estimated "~30 occurrences of `t-esc` to fix". Actual count was 726 across 33 files (24× under-estimate). Author counted only the lines visible in one diff snippet and projected.

**Rule:** when sizing a debug investigation ("how widespread is this bug?"), MANDATORY pre-flight is `rg --count <pattern>` over the **full module tree**, not just the cited file. Print "estimate vs actual + ratio" before writing the bug summary.

```bash
# default pattern for any "how many places" debug step
rg --count -F '<failing-pattern>' <module-root>

# OR with ast-grep for structural matches
ast-grep -p '<failing-pattern>' --json <module-root> | jq length
```

**Apply:** when the user says "this is happening in a few places" or "we estimate ~N occurrences", do not start fixing until the global grep count is on disk. Companion skill: `estimate-grep` (slash command: `/estimate-grep`).

### AP-2 — don't trust upstream rule blindly

**Phase 28 anchor:** D1 — `~/.claude/rules/odoo/odoo.md` Section 2 said `<tree>` root deprecated → use `<list>`. Bundle B fixer applied this; `-u cap_purchase_extended` raised `ParseError: type=tree but root is <list>`. The actual Odoo 17 source said the opposite.

**Rule:** when a bug investigation routes through a `~/.claude/rules/*` rule, an audit doc, or a skill reference, treat the rule as a **claim**, not a fact. Verify against the upstream source (Odoo source for Odoo bugs; library source for library bugs) before applying it as a fix. Refuse rules that say "X is deprecated" / "always Y" without an upstream `repo:line-range` citation. Companion skill: `upstream-rule-verify` (slash command: `/upstream-rule-verify`).

### AP-3 Re-verify on use — audit-doc counts decay

**Phase 28 anchor:** F1 — Layer A 2026-05-06 stamp count of 15,604 in audit doc; live DB 13,903.

**Rule:** when a debug session reads a number from a previous SUMMARY.md / VERIFICATION.md / audit doc, re-derive against the live DB before taking action. Older than 24h = stale. Companion skill: `audit-doc-freshness`.
