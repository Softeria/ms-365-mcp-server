...
### AP-1 State filter scope inline on every metric

**Phase 28 anchor:** A2 — HANDOFF v1.7 said "97.4% → 61.73% — 40,122 stamps regressed". The 97.4% was VENDCM-only move-level; the 61.73% was full-D4 line-level. Apples-oranges; no regression. The doc's bare percentages caused the false-regression conclusion to propagate for days.

**Rule:** every metric (count, %, sum, AED amount, coverage) written into a doc MUST carry, **inline in the same sentence**:

| Element | What to include |
|---|---|
| `(numerator/denominator)` | absolute values, not just % |
| Granularity | move-level / line-level / record-level |
| Sub-domain filter | the explicit WHERE clause or named filter |
| Time window | as-of date, or the live-DB query timestamp |

**Examples:**

| Wrong | Right |
|---|---|
| "97.4% partner coverage" | "VENDCM move-level partner coverage: 12,345 / 12,678 (97.4%) as of 2026-05-08" |
| "61.73% coverage" | "Full-D4 line-level coverage: 40,122 / 65,000 (61.73%) — different axis from VENDCM move-level above; not comparable" |
| "stamped 15,604 moves" | "stamped `oracle_dr_cr_hd_id` on 15,604 / 24,707 D4 NULL moves (63.2%) as of 2026-05-06; **re-verify on use** with: `SELECT COUNT(*) FROM account_move WHERE oracle_dr_cr_hd_id IS NOT NULL`" |

**Apply:** before approving any doc that contains a number, scan every metric for these four elements. If any is missing, raise as `BARE_METRIC` and refuse approval. Companion skill: `metric-filter-inline` (slash command: `/metric-filter-inline`).

### AP-2 — `Re-verify on use` stamp on stamp-counts

**Phase 28 anchor:** F1 — Layer A 2026-05-06 stamp count of 15,604 in audit doc; live DB later showed 13,903 (1,701 drift). Counts decay.

**Rule:** every stamp-count or post-action count written into a doc must end with `**Re-verify on use**` + the SQL that reproduces it. This makes the doc self-describing — any future reader knows how to regenerate the number rather than trusting the stale value.

**Apply:** in the doc-writer template's `## Migration / Phase metrics` (or equivalent) section, every count must be followed by the recomputation snippet. Companion skill: `audit-doc-freshness`.

### AP-3 Don't propagate "X is deprecated" without an upstream citation

**Phase 28 anchor:** D1 — `~/.claude/rules/odoo/odoo.md` said `<tree>` was deprecated; Odoo 17 source said the opposite. The rule had no upstream citation; nobody re-verified before propagating.

**Rule:** when writing a doc that says "X is deprecated" / "always use Y" / "X is canonical", include the upstream source citation (URL + line range OR commit hash + 5-line excerpt). Refuse to write the deprecation claim without it. Companion skill: `upstream-rule-verify`.
