...
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/mandatory-initial-read.md
...
<required_reading>
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/verification-overrides.md
...
**Project skills:** @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/project-skills-discovery.md
- Load `rules/*.md` as needed during **verification**.
- Apply skill rules when scanning for anti-patterns and verifying quality.
</project_context>

<graphify_first>
Before broad source reads or raw search, resolve `REPO_ROOT=$(git rev-parse --show-toplevel)`. If `$REPO_ROOT/graphify-out/GRAPH_REPORT.md` exists, read it once after project context. Use `graphify query`, `graphify path`, or `graphify explain` from `$REPO_ROOT` to identify likely artifacts, wiring, and cross-module relationships before `Grep`, `Glob`, `rg`, or `find`; raw search verifies exact evidence.
</graphify_first>
...
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/few-shot-examples/verifier.md
...
**When the phase under verification has `mode: mvp` in ROADMAP.md (resolved by the verify-work workflow):** Apply the goal-backward methodology, narrowed to the phase's user-story goal. Required reading: `@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/verify-mvp-mode.md`.
...
## Anti-patterns to avoid (Phase 28 forensics 2026-05-08)

These are anchored to real mistakes in `.planning/phases/28-clean-reimport/AUDIT-2026-05-08/learnings/SESSION-MISTAKES-CATALOGUE.md`. Apply to every verification.

### AP-1 Handoff-freshness — never re-quote a count without re-deriving it

**Phase 28 anchor:** B3 — audit doc cited 15,604 stamps; live DB showed 13,903 (1,701 drift). F1 — Layer A 2026-05-06 wrong-stamps incident re-fired conceptually as the same stamp-count drift.

**Rule:** any count, percentage, or coverage number that the verifier reads from a SUMMARY.md / HANDOFF.json / audit doc MUST be re-derived against the live DB before it is allowed into the VERIFICATION.md `Evidence` column. Older than 24 hours = stale by default.

**Apply:** when an audit doc says "stamped 15,604 moves on `oracle_dr_cr_hd_id`", run the live SQL `SELECT COUNT(*) FROM account_move WHERE oracle_dr_cr_hd_id IS NOT NULL AND state='posted'` before quoting. Companion skill: `audit-doc-freshness` (slash command: `/audit-doc-freshness`).

### AP-2 Apples-to-oranges — every coverage % carries (numerator/denominator) inline

**Phase 28 anchor:** A2 — HANDOFF v1.7 framed a partner gap as "97.4% → 61.73% — 40,122 stamps regressed". The 97.4% was VENDCM-only move-level; the 61.73% was full-D4 line-level. No regression. Apples-oranges.

**Rule:** when a verifier evaluates a coverage truth, refuse to quote a bare %. Restate as "VENDCM move-level: 12,345 / 12,678 (97.4%); full-D4 line-level: 40,122 / 65,000 (61.73%)" — different axes, no comparison.

**Apply:** under `### Required Artifacts` and `### Key Link Verification` in VERIFICATION.md, every metric in the `Details` column must include numerator + denominator + the WHERE filter. Companion skill: `metric-filter-inline` (slash command: `/metric-filter-inline`).

### AP-3 Audit-doc decay — `git log` proof for every "still open" CRITICAL

**Phase 28 anchor:** B1 + B2 — audit docs claimed CRITICAL field-decl collision and account_id=988 hardcode "still open"; both were already fixed on disk. Audit doc was several days stale.

**Rule:** before flagging an artifact as MISSING, STUB, or NOT_WIRED based on an audit-doc claim that says "CRITICAL — still open", run:

```bash
git log -1 --since=<audit-date> -- <file>
```

If the file changed after the audit was authored, mark "needs re-verify" and re-grep the file in the current state. Do NOT propagate the audit's "still open" verdict.

**Apply:** under `## Step 0: Check for Previous Verification` and during `## Step 7: Scan for Anti-Patterns`, when the previous VERIFICATION.md / SUMMARY.md cites a CRITICAL by file path, run the git-log freshness check first.

### AP-4 — refuse FK / column claims under 90% populated coverage

**Phase 28 anchor:** A3 — brief proposed `oracle_dr_cr_hd_id` as the join FK; it was 17.3% populated overall, 0% on VENDADJ. The IH-chain via `oracle_je_header_id` was 100% populated.

**Rule:** when a truth or key-link is "joined via column X", run:

```sql
SELECT COUNT(*) AS total, COUNT(<col>) AS populated, ROUND(100.0*COUNT(<col>)/COUNT(*),2) AS pct
FROM <table> WHERE <universe_filter>;
```

If `pct < 90`, mark the truth as FAILED with reason "FK coverage <90% — bridge column probably wrong; propose alternative". Do NOT mark it VERIFIED on the basis that "the column exists".
