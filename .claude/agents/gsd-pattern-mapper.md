...
### AP-1 Estimate-grep-first — every "this pattern appears in N files" claim must be grepped

**Phase 28 anchor:** A4 — UI-gap audit said "~30 occurrences of `t-esc` to fix". Actual count was 726 across 33 files (24× under-estimate).

**Rule:** when PATTERNS.md cites a "this pattern appears N times" claim — whether for the analog match, the spread of an anti-pattern, or the size of a refactor sweep — the N must come from `rg --count <pattern>` over the full module tree, NOT from inspection of one file.

```bash
# canonical pre-flight before claiming "N occurrences"
rg --count -F '<pattern>' <repo-root> | awk -F: '{sum+=$2} END {print sum}'
```

When PATTERNS.md emits "candidate analogs", include the global occurrence count for each analog so the planner can size effort.

Companion skill: `estimate-grep` (slash command: `/estimate-grep`).

### AP-2 — don't trust the rule blindly

**Phase 28 anchor:** D1 — Bundle B fixer trusted (rules library not shipped in cloud) "always use `<list>`" rule and applied it; the rule was wrong vs Odoo 17 source.

**Rule:** when PATTERNS.md surfaces a "canonical pattern" anchored to a (rules library not shipped in cloud) document, include in the patterns table the upstream source citation that proves the rule. If no upstream citation exists, mark the pattern as "RULE_UNVERIFIED" and route to `upstream-rule-verify`.
