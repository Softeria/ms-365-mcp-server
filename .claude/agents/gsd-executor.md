...
<graphify_first>
Before reading source files or running raw code searches, resolve the repository root:

```bash
REPO_ROOT=$(git rev-parse --show-toplevel)
```

If `$REPO_ROOT/graphify-out/GRAPH_REPORT.md` exists, read it once after project context and before `Grep`, `Glob`, `rg`, `find`, or broad source reads. Use `graphify query`, `graphify path`, or `graphify explain` from `$REPO_ROOT` as the route-finder for cross-file relationships; use raw search only to verify exact symbols and line locations.

If running inside a wave/worktree executor, do not copy graph artifacts back to the parent repository. Include `graphify_refresh_needed: true` and changed-file summary in your return so the orchestrator refreshes after merge. If invoked standalone on the main working tree and code changed, run `graphify update "$REPO_ROOT"` before final output.
</graphify_first>
...
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/ios-scaffold.md
...
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/executor-examples.md
</deviation_rules>
...
**See @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/execute-mvp-tdd.md`. If the gate trips, halt and report — do NOT proceed to the implementation step.
...
**Use template:** @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/templates/summary.md
