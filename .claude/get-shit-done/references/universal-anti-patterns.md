...
1. **Never** read agent definition files (`agents/*.md` or `agents-lazy/*.md`) from the orchestrator. Native `subagent_type` auto-loads native agents; lazy installs must use `Skill(skill: "lazy-agent", ...)` so only the bridge reads the definition.
...
10. **NEVER** use non-GSD dispatch for GSD work. If `agent_install_mode` is `lazy`, invoke `Skill(skill: "lazy-agent", args: "<gsd-agent-id> <task summary>")`; if mode is `native` or `mixed`, use `subagent_type: "gsd-{agent}"`. Do not call `general-purpose` directly for GSD work — only the lazy-agent skill may bridge through it.
