...
<agent_dispatch>
Before spawning any `gsd-*` agent, read `agent_install_mode` from init JSON. If mode is `lazy`, invoke `Skill(skill: "lazy-agent", args: "<agent-id> <task summary>")`; the lazy-agent bridge must make the child read its own agent file and must not load the full definition into coordinator context. Use native `Agent(subagent_type="gsd-*", ...)` only when mode is `native` or `mixed`. Never call `general-purpose` directly for GSD work; `lazy-agent` is the only allowed bridge to `general-purpose`.
</agent_dispatch>
...
**Fallback rule:** If a spawned agent completes its work (commits visible, SUMMARY.md exists, or an explicit final report is available) but
...
via filesystem, git state, and the agent's final report when available.
</runtime_compatibility>

<required_reading>
Context budget rule: SDK inventory first, docs second only when needed.

Before reading broad docs, run `gsd-sdk query init.execute-phase "${PHASE_ARG}"` and `gsd-sdk query phase-plan-index "${PHASE_NUMBER}"` once phase number is known. Use those outputs for state, phase directory, plan counts, incomplete plans, agent install mode, and config. Do not read full STATE.md, ROADMAP.md, REQUIREMENTS.md, graph reports, or workflow reference files in the coordinator unless the SDK output is missing, ambiguous, or a specific gate requires source verification.

Reference files below are on-demand, not mandatory preload:
- @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/agent-contracts.md
- @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/context-budget.md
- @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/gates.md
</required_reading>
...
Load inventory in one call before reading any phase documents:
...
Parse JSON for: `agent_install_mode`, `executor_model`, `verifier_model`, `commit_docs`, `parallelization`, `branching_strategy`, `branch_name`, `phase_found`, `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `plans`, `incomplete_plans`, `plan_count`, `incomplete_count`, `state_exists`, `roadmap_exists`, `phase_req_ids`, `response_language`.

Then query `gsd-sdk query phase-plan-index "${PHASE_NUMBER}"` and treat it as the coordinator's primary progress source. Only read incomplete plan files, current `.continue-here.md`, and summary files needed for gate decisions. Subagents own full plan/workflow/doc reading.
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/workflows/execute-plan.md` **inline**
...
Parse JSON for: `agent_install_mode`, `phase`, `plans[]` (each with `id`, `wave`, `autonomous`, `objective`, `files_modified`, `task_count`, `has_summary`), `waves` (map of wave number → plan IDs), `incomplete`, `has_checkpoints`.
...
3. **Spawn executor agents:**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       Graphify-first: resolve `REPO_ROOT=$(git rev-parse --show-toplevel)`. If `$REPO_ROOT/graphify-out/GRAPH_REPORT.md` exists, read it before raw source reads or searches. Use `graphify query`, `graphify path`, or `graphify explain` as route-finder before `Grep`, `Glob`, `rg`, or `find`; raw search only verifies exact code locations. Do not refresh graph artifacts from inside this worktree. Return `graphify_refresh_needed: true` with changed files; the orchestrator refreshes after wave merge.
       ...
       <execution_context>
       @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/workflows/execute-plan.md
       @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/templates/summary.md
       @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/checkpoints.md
       @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/tdd.md
       @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/executor-examples.md'}
       </execution_context>
   ...
   **Sequential mode** (`USE_WORKTREES_FOR_PLAN` is `false` — either project-level `USE_WORKTREES=false`, or per-plan submodule intersection forced it false in step 2.5):
   ...
   ```
       ...
       Graphify-first: resolve `REPO_ROOT=$(git rev-parse --show-toplevel)`. If `$REPO_ROOT/graphify-out/GRAPH_REPORT.md` exists, read it before raw source reads or searches. Use `graphify query`, `graphify path`, or `graphify explain` as route-finder before `Grep`, `Glob`, `rg`, or `find`; raw search only verifies exact code locations. If code changed, run `graphify update "$REPO_ROOT"` before final output and note whether refresh passed.
       REQUIRED ORDER: Write SUMMARY.md → commit → only then any narration. No text between Write and commit (truncation risk; #2070 rescue is not primary defense).
       </sequential_execution>
   ```
...
5.8. **Graphify refresh after wave changes:** If graphify is enabled and the wave produced code changes, refresh the main repository graph after all worktree merges or sequential plan completion, before verification/review gates:
   ```bash
   GRAPHIFY_ENABLED=$(gsd-sdk query config-get graphify.enabled 2>/dev/null || echo "false")
   if [ "$GRAPHIFY_ENABLED" = "true" ] && [ -f graphify-out/graph.json ]; then
     graphify update . || echo "⚠ graphify update failed — run /gsd-graphify build before verification"
   fi
   ```
   For very large phases or stale `.planning/graphs`, run `/gsd-graphify build` after the final wave to sync GSD graph artifacts.

5.9. **Handle test gate failures (when `WAVE_FAILURE_COUNT > 0`):**
...
Read and follow `${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
