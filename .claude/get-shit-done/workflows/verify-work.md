...
<template>
@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/templates/UAT.md
</template>
...
Parse JSON for: `agent_install_mode`, `planner_model`, `checker_model`, `commit_docs`, `phase_found`, `phase_dir`, `phase_number`, `phase_name`, `has_verification`, `uat_path`.
...
**MVP-mode UAT framing.** When `MVP_MODE=true`, follow the rules in `@${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/references/verify-mvp-mode.md`. Briefly:
...
Read and follow `${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/workflows/transition.md`.
...
- Follow @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/workflows/diagnose-issues.md
