...
> `${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/USER-PROFILE.md` exists). Skip the Read
...
```bash
PROFILE_CONTENT=$(cat "${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
