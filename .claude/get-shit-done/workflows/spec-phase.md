...
```bash
INIT=$(node "${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
...
Use the SPEC.md template from @${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}/.claude/get-shit-done/templates/spec.md.
