...
Archive accumulated phase directories from completed milestones into `.planning/milestones/v{X.Y}-phases/`. Identifies which phases belong to each completed milestone, shows a dry-run summary, and moves directories on confirmation. Cleanup must be evidence-based: verify membership and current references before moving or deleting artifacts.
...
Match phase directories to milestone membership. Only include directories that still exist in `.planning/phases/`. Before proposing a move, check current references so active handoffs, state files, or in-progress tasks are not archived as stale cleanup.
