<purpose>
Retroactive 6-pillar visual audit of implemented frontend code. Standalone command that works on any project — GSD-managed or not. Produces scored UI-REVIEW.md with actionable findings. Visual judgments must come from screenshots; code, DOM, and text inspection may support coverage/navigation only.
</purpose>
...
{If UI-SPEC exists: "Audit against UI-SPEC.md design contract and current target standard."}
{If no UI-SPEC: "Audit against abstract 6-pillar standards and current-product quality."}
{Prototype parity is required only when the user explicitly requests prototype parity.}
</objective>
...
tools is detected at runtime. Visual/design findings still require screenshot evidence.
