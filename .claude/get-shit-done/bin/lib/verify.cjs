...
function cmdValidateHealth(cwd, options, raw) {
  ...
  try {
    ...
    if (!agentStatus.agents_installed) {
      if (agentStatus.lazy_agent_skill_required && !agentStatus.lazy_agent_skill_installed) {
        addIssue('warning', 'W010',
          `GSD lazy agents found in ${agentStatus.lazy_agents_dir}, but lazy-agent skill is missing at ${agentStatus.lazy_agent_skill_dir}`,
          'Install or restore ~/.claude/skills/lazy-agent/SKILL.md');
      } else if (agentStatus.installed_agents.length === 0) {
        addIssue('warning', 'W010',
          `No GSD agents found in ${agentStatus.agents_dir} or ${agentStatus.lazy_agents_dir}`,
          'Run the GSD installer: npx get-shit-done-cc@latest');
      } else {
        addIssue('warning', 'W010',
          `Missing ${agentStatus.missing_agents.length} GSD agents across native and lazy installs: ${agentStatus.missing_agents.join(', ')}`,
          'Run the GSD installer or restore the missing files in agents-lazy/');
      }
    }
...
function cmdValidateAgents(cwd, raw) {
  ...
  output({
    ...
    lazy_agents_dir: agentStatus.lazy_agents_dir,
    agent_install_mode: agentStatus.agent_install_mode,
    lazy_agent_skill_dir: agentStatus.lazy_agent_skill_dir,
    lazy_agent_skill_installed: agentStatus.lazy_agent_skill_installed,
    lazy_agent_skill_required: agentStatus.lazy_agent_skill_required,
    agents_found: agentStatus.agents_installed,
    installed: agentStatus.installed_agents,
    native: agentStatus.native_agents,
    lazy: agentStatus.lazy_agents,
