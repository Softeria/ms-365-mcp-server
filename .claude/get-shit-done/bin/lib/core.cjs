...
/**
 * Resolve the eager-loaded agents directory from the GSD install location.
...
/**
 * Resolve the lazy-loaded agents directory from the GSD install location.
 *
 * GSD_LAZY_AGENTS_DIR env var overrides the default path. Lazy agents are used
 * by the lazy-agent skill so Claude Code does not eager-load every GSD prompt.
 *
 * @returns {string} Absolute path to the lazy agents directory
 */
function getLazyAgentsDir() {
  if (process.env.GSD_LAZY_AGENTS_DIR) {
    return process.env.GSD_LAZY_AGENTS_DIR;
  }
  return path.join(__dirname, '..', '..', '..', 'agents-lazy');
}

/**
 * Resolve the lazy-agent skill directory from the GSD install location.
 *
 * GSD_LAZY_AGENT_SKILL_DIR env var overrides the default path. The skill must
 * exist for lazy-loaded agents to be invokable from Claude Code workflows.
 *
 * @returns {string} Absolute path to the lazy-agent skill directory
 */
function getLazyAgentSkillDir() {
  if (process.env.GSD_LAZY_AGENT_SKILL_DIR) {
    return process.env.GSD_LAZY_AGENT_SKILL_DIR;
  }
  return path.join(__dirname, '..', '..', '..', 'skills', 'lazy-agent');
}

function hasAgentFile(dir, agent) {
  if (!fs.existsSync(dir)) return false;
  const agentFile = path.join(dir, `${agent}.md`);
  const agentFileCopilot = path.join(dir, `${agent}.agent.md`);
  return fs.existsSync(agentFile) || fs.existsSync(agentFileCopilot);
}
 ...
 *
 * Recognises standard format (gsd-planner.md), Copilot format
 * (gsd-planner.agent.md), and lazy-agent format in agents-lazy/.
 *
 * @returns {{ agents_installed: boolean, missing_agents: string[], installed_agents: string[], agents_dir: string, lazy_agents_dir: string, agent_install_mode: string, lazy_agent_skill_installed: boolean }}
 */
function checkAgentsInstalled() {
  const agentsDir = getAgentsDir();
  const lazyAgentsDir = getLazyAgentsDir();
  const lazyAgentSkillDir = getLazyAgentSkillDir();
  const expectedAgents = Object.keys(MODEL_PROFILES);
  const installed = [];
  const nativeInstalled = [];
  const lazyInstalled = [];
  const missing = [];
  const lazyAgentSkillInstalled = fs.existsSync(path.join(lazyAgentSkillDir, 'SKILL.md'));

  for (const agent of expectedAgents) {
    const hasNative = hasAgentFile(agentsDir, agent);
    const hasLazy = hasAgentFile(lazyAgentsDir, agent);

    if (hasNative) nativeInstalled.push(agent);
    if (hasLazy) lazyInstalled.push(agent);

    if (hasNative || hasLazy) {
  ...
  let agentInstallMode = 'none';
  if (nativeInstalled.length > 0 && lazyInstalled.length > 0) {
    agentInstallMode = 'mixed';
  } else if (nativeInstalled.length > 0) {
    agentInstallMode = 'native';
  } else if (lazyInstalled.length > 0) {
    agentInstallMode = 'lazy';
  }

  const nativeAgentSet = new Set(nativeInstalled);
  const lazyAgentSkillRequired = lazyInstalled.some((agent) => !nativeAgentSet.has(agent));

  return {
    agents_installed: installed.length > 0 && missing.length === 0 && (!lazyAgentSkillRequired || lazyAgentSkillInstalled),
    missing_agents: missing,
    installed_agents: installed,
    native_agents: nativeInstalled,
    lazy_agents: lazyInstalled,
    agents_dir: agentsDir,
    lazy_agents_dir: lazyAgentsDir,
    agent_install_mode: agentInstallMode,
    lazy_agent_skill_dir: lazyAgentSkillDir,
    lazy_agent_skill_installed: lazyAgentSkillInstalled,
    lazy_agent_skill_required: lazyAgentSkillRequired,
  };
}

function addAgentStatusToResult(result) {
  const agentStatus = checkAgentsInstalled();
  result.agents_installed = agentStatus.agents_installed;
  result.missing_agents = agentStatus.missing_agents;
  result.installed_agents = agentStatus.installed_agents;
  result.native_agents = agentStatus.native_agents;
  result.lazy_agents = agentStatus.lazy_agents;
  result.agents_dir = agentStatus.agents_dir;
  result.lazy_agents_dir = agentStatus.lazy_agents_dir;
  result.agent_install_mode = agentStatus.agent_install_mode;
  result.lazy_agent_skill_dir = agentStatus.lazy_agent_skill_dir;
  result.lazy_agent_skill_installed = agentStatus.lazy_agent_skill_installed;
  result.lazy_agent_skill_required = agentStatus.lazy_agent_skill_required;
  return result;
}
...
module.exports = {
  ...
  getAgentsDir,
  getLazyAgentsDir,
  getLazyAgentSkillDir,
  checkAgentsInstalled,
  addAgentStatusToResult,
  atomicWriteFileSync,
  timeAgo,
