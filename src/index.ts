...
async function main(): Promise<void> {
  try {
    ...
    if (args.userOnly) {
      logger.info('User-only mode enabled - filtering out admin-consent-required scopes');
    }

    const scopes = buildScopesFromEndpoints(includeWorkScopes, args.enabledTools, args.userOnly);
